    ...
    private fun startLocation() {
        if (checkSelfPermission(this, ACCESS_FINE_LOCATION) != PERMISSION_GRANTED
            ...
            ActivityCompat.requestPermissions(this,
                arrayOf(ACCESS_FINE_LOCATION, ACCESS_COARSE_LOCATION), LOCATION_PERMISSION_REQUEST_CODE
            )
            return
        }
        ...
        bestLocation?.let {
            ...
            val latLng = LatLng(it.latitude, it.longitude)
            map.addMarker(MarkerOptions().position(latLng).title("My network position"))
            map.moveCamera(CameraUpdateFactory.newLatLngZoom(latLng, 17F))

            map.addPolygon(PolygonOptions().add(latLng))
        }

        locationManager.requestLocationUpdates(locationProvider, 2000, 2F, this)
    }
